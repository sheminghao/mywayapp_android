...
//    PhotoAdapter photoAdapter;
    ...
    public void attachView(TrackResultView view) {
        ...
        recyclerView = mView.getPhotoRecycler();
        selectPhoto();
    }
...
//        adapter.setSelectMax(9);
        ...
        adapter.setOnItemClickListener(new GridOnlyImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                // 这里可预览图片
                PictureConfig.getInstance().externalPicturePreview((Activity)mContext, position, routeImg);
            }
        });
    }
...
//                    config.setCheckNumMode(isCheckNumMode);
                    config.setCompressQuality(100);
                    config.setImageSpanCount(4);
                    config.setSelectMedia(routeImg);
    ...
    private PictureConfig.OnSelectResultCallback resultCallback = new PictureConfig.OnSelectResultCallback() {
        ...
        public void onSelectSuccess(List<LocalMedia> resultList) {
            ...
            if (null != resultList) {
                if (mode == FunctionConfig.MODE_SINGLE) {
                    coverImg = resultList;
