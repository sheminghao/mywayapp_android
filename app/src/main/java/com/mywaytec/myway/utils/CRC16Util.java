...
public class CRC16Util {
    ...
    static byte[] crc16_tab_l = {
            ...
            0x3C, (byte)0xFC, (byte)0xFD, 0x3D, (byte)0xFF, (byte)0x3F, 0x3E, (byte)0xFE, (byte)0xFA,
            0x3A, 0x3B, (byte)0xFB, 0x39, (byte)0xF9, (byte)0xF8, 0x38, 0x28, (byte)0xE8, (byte)0xE9,
            0x29, (byte)0xEB, 0x2B, 0x2A, (byte)0xEA, (byte)0xEE, 0x2E, 0x2F, (byte)0xEF, 0x2D, (byte)0xED,
     ...
     * @return
     */
    public static char updateCrc(char crcIn, byte b)
    {
        int crc = crcIn;
        int in = b|0x100;

        do {
//            Log.i("TAG", "------Ymodem CRC校验,updateCrc");
            crc <<= 1;
            in <<= 1;
            if ((in&0x100) == 0x100)
                ++crc;
     ...
     * @return
     */
    public static char calCrc16(byte[] dataAll,byte[] data, int size){
        int crc = 0;
//        byte[] dataEnd = new byte[size];
//        dataEnd[0] = dataAll[dataAll.length-2];
//        while (dataAll[0]<dataEnd[0]){
//            crc = updateCrc((char) crc, dataAll[tag]);
//            tag++;
//            Log.i("TAG", "------Ymodem CRC校验,calCrc16");
//        }
        for (int i = 0; i < size; i++) {
            crc = updateCrc((char) crc, data[i]);
//            Log.i("TAG", "------CRC校验：crc,"+crc+",i,"+i+",data[i],"+data[i]);
        }
        crc = updateCrc((char) crc, (byte) 0);
        crc = updateCrc((char) crc, (byte) 0);
        return (char)(crc&0xffff);
